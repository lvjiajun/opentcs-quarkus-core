...
import org.youbai.opentcs.util.Colors;
import org.youbai.opentcs.util.persistence.v003.V003ModelParser;
import org.youbai.opentcs.util.persistence.v003.V003PlantModelTO;
import org.youbai.opentcs.util.persistence.v003.VisualLayoutTO.ModelLayoutElement;
import org.youbai.opentcs.data.ObjectPropConstants;
...
public class V004ModelParser {
  ...
  private List<PropertyTO> convertProperties(
      List<org.youbai.opentcs.util.persistence.v003.PropertyTO> tos) {
    return tos.stream()
  ...
  private Map<String, String> toPropertiesMap(
      List<org.youbai.opentcs.util.persistence.v003.PropertyTO> properties) {
    Map<String, String> result = new HashMap<>();
    for (org.youbai.opentcs.util.persistence.v003.PropertyTO property : properties) {
  ...
  private List<PointTO.OutgoingPath> convertOutgoingPaths(
      org.youbai.opentcs.util.persistence.v003.PointTO to) {
    ...
    return to.getLocationTypes().stream()
        .map(locationType -> {
          String locationRepresentation = toPropertiesMap(locationType.getProperties())
              .getOrDefault(ObjectPropConstants.LOCTYPE_DEFAULT_REPRESENTATION, LocationRepresentation.NONE.name());
  ...
  private List<AllowedOperationTO> convertAllowedOperations(
      List<org.youbai.opentcs.util.persistence.v003.AllowedOperationTO> tos) {
    return tos.stream()
    ...
    return to.getLocations().stream()
        .map(location -> {
          ...
          String locationRepresentation = toPropertiesMap(location.getProperties())
              .getOrDefault(ObjectPropConstants.LOC_DEFAULT_REPRESENTATION, LocationRepresentation.DEFAULT.name());
  ...
  private List<LocationTO.Link> convertLinks(org.youbai.opentcs.util.persistence.v003.LocationTO to) {
  ...
  private List<MemberTO> convertMembers(List<org.youbai.opentcs.util.persistence.v003.MemberTO> tos) {
    return tos.stream()
        .map(member -> {
