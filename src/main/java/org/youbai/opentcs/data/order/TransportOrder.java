 ...
 */
package org.youbai.opentcs.data.order;
...
public class TransportOrder
  ...
  public TransportOrder(String name, List<DriveOrder> driveOrders) {
    super(name,
          new HashMap<>(),
          new ObjectHistory().withEntryAppended(new ObjectHistory.Entry(TransportOrderHistoryCodes.ORDER_CREATED)));
  ...
  private ObjectHistory historyForNewState(State state) {
    return state.isFinalState()
        ? getHistory().withEntryAppended(new ObjectHistory.Entry(TransportOrderHistoryCodes.ORDER_REACHED_FINAL_STATE))
        : getHistory();
  }

  private ObjectHistory historyForNewDriveOrderState(DriveOrder.State state) {
    return state == DriveOrder.State.FINISHED
        ? getHistory().withEntryAppended(new ObjectHistory.Entry(TransportOrderHistoryCodes.ORDER_DRIVE_ORDER_FINISHED))
        : getHistory();
  }
