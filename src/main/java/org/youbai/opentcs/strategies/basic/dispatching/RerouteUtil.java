...
public class RerouteUtil {
   ...
   */
  public List<Route.Step> getFutureSteps(Vehicle vehicle) {
    ...
    List<Route.Step> currentSteps = order.getCurrentDriveOrder().getRoute().getSteps();
    ...
    List<Route.Step> futureSteps = new ArrayList<>();
  ...
  private List<Route.Step> mergeSteps(List<Route.Step> stepsA, List<Route.Step> stepsB) {
  ...
  private List<DriveOrder> updatePathLocks(List<DriveOrder> orders) {
    ...
    for (DriveOrder order : orders) {
      ...
      for (Route.Step step : order.getRoute().getSteps()) {
  ...
  private List<DriveOrder> markRestrictedSteps(List<DriveOrder> orders,
                                               Predicate<Route.Step> executionTest) {
    if (configuration.reroutingImpossibleStrategy() == DefaultDispatcherConfiguration.ReroutingImpossibleStrategy.IGNORE_PATH_LOCKS) {
      return orders;
    }
    ...
    for (DriveOrder order : orders) {
      ...
      for (Route.Step step : order.getRoute().getSteps()) {
        ...
        updatedSteps.add(new Route.Step(step.getPath(),
                                  step.getSourcePoint(),
                                  step.getDestinationPoint(),
                                  step.getVehicleOrientation(),
                                  step.getRouteIndex(),
  ...
  private class ExecutionTest
      implements Predicate<Route.Step> {
     ...
     */
    public ExecutionTest(DefaultDispatcherConfiguration.ReroutingImpossibleStrategy strategy, Point source) {
